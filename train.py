...
def main_worker(gpu, args):
    ...
    #++++++++++++++++++++
    '''
    net = model.inpaint_model
    input_tensor = torch.randn(1, 3, 256, 256).to('cuda:0')
    input_mask = torch.randn(1, 1, 256, 256).to('cuda:0')
    batch = {
            'image': input_tensor,  # 形状为 (N, C, H, W)
            'mask': input_mask    # 形状为 (N, C, H, W)
        }
