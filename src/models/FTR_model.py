...
class LaMaBaseInpaintingTrainingModule(nn.Module):
    ...
    def load(self):
        if self.test:
            ...
            '''
            with open("/data/yc/ZITS/generator.txt", "w") as f:
                for key in self.generator.state_dict().keys():
                    f.write(key + "\n")

            with open("/data/yc/ZITS/data.txt", "w") as f:
...
class BaseInpaintingTrainingModule(nn.Module):
    def __init__(self, config, gpu, name, rank, *args, test=False, **kwargs):
        ...
        if self.config.optimizers.get('decay_steps', 0)  is not None and self.config.optimizers.get('decay_steps', 0) > 0 and not test:
