...
class LaMa:
    def __init__(self, config, gpu, rank, test=False):
        ...
        self.train_dataset = ImgDataset(config.TRAIN_FLIST, config.INPUT_SIZE, config.MASK_RATE, config.IRREGULAR_MASK_FLIST,config.COCO_MASK_FLIST,
        ...
        self.val_dataset = ImgDataset(config.VAL_FLIST, config.INPUT_SIZE, mask_rates=None, irregular_mask_path=None, coco_mask_path=None,augment=False,
    ...
    def sample(self, it=None):
        ...
        #+++++++++++++++++++++
        '''
        save_dir = os.path.join(self.samples_path, self.model_name, f"iter_{it}")
        os.makedirs(save_dir, exist_ok=True)

        def save_image(tensor, filename):
            img = stitch_images(self.postprocess(tensor.cpu()), img_per_row=1)  # 转为PIL图像
            img.save(os.path.join(save_dir, filename))

        save_image(items['image'], "original.png")            # 原始图像
        save_image(inputs, "masked_input.png")               # masked输入
        save_image(items['mask'], "mask.png")                # 掩码
        save_image(items['predicted_image'], "predicted.png") # 修复结果
        save_image(outputs_merged, "merged_output.png")      # 最终合并结果

        print(f'\nSaved samples to: {save_dir},iteration:{iteration}')
